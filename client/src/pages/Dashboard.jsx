...
export default function Dashboard() {
  ...
  const handleJoin = async e => {
    ...
    setJoinError('')
    try {
      await sessionAPI.join({ roomCode: joinCode.trim() })
  ...
  return (
    <div className="page">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '14px', marginBottom: '32px' }}>
        {[
          { label: 'Total P/L', value: formatINR(user.totalProfit || 0), color: user.totalProfit >= 0 ? '#22c55e' : '#ef4444' },
        ...
        <>
            ...
            <section style={{ marginBottom: '32px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '14px' }}>
                {activeSessions.map(s => (
                  <SessionCard key={s._id} session={s} userId={user._id} />
                ))}
              </div>
            </section>
          )}
          ...
          {sessions.length === 0 && (
            <div style={{ textAlign: 'center', padding: '80px 20px', color: 'var(--text-muted)' }}>
              ...
              <Link to="/session/create" className="btn btn-primary">♠ Create Session</Link>
            </div>
          )}
        </>
      )}
...
function SessionCard({ session, userId }) {
  ...
  return (
    <Link to={`/session/${session.roomCode}`} style={{ textDecoration: 'none' }}>
